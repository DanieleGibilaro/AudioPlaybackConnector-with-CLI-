...
#include <shellapi.h> // Per CommandLineToArgvW
#include <iostream>   // Per output a console
...
winrt::fire_and_forget ListDevicesAsync(); // Funzione asincrona separata

// Variabile globale per memorizzare i comandi da riga di comando
std::vector<std::wstring> g_commandLineDevices;
bool g_autoConnect = false;
bool g_listDevices = false;
bool g_hasConsole = false; // Nuova flag per verificare se abbiamo una console

// Definizioni delle funzioni mancanti
void SetupFlyout()
{
    TextBlock textBlock;
...
// Funzione per verificare se il processo ha una console
bool HasConsole()
{
...
// Funzione asincrona per listare i dispositivi disponibili
winrt::fire_and_forget ListDevicesAsync()
{
    try
    {
        // Verifica se abbiamo già una console (chiamato da cmd/powershell)
        if (!g_hasConsole)
        {
            // Se non abbiamo una console, ne attacchiamo una (per debug)
        ...
        // Usa stdout/stderr invece di wcout per compatibilità
        fprintf(stdout, "Ricerca dispositivi audio Bluetooth...\n\n");

        // Ottieni il selettore per i dispositivi audio playback
        auto selector = AudioPlaybackConnection::GetDeviceSelector();
        auto deviceCollection = co_await DeviceInformation::FindAllAsync(selector);

        if (deviceCollection.Size() == 0)
        {
            fprintf(stdout, "Nessun dispositivo audio Bluetooth trovato.\n");
        }
        else
        {
            //fprintf(stdout, "Dispositivi disponibili (%zu):\n\n", deviceCollection.Size());

            for (uint32_t i = 0; i < deviceCollection.Size(); ++i)
            {
                auto device = deviceCollection.GetAt(i);
                auto deviceName = device.Name();
                auto deviceId = device.Id();

                fprintf(stdout, "%ls@",  deviceName.c_str());
                fprintf(stdout, "%ls\n", deviceId.c_str());

                // Verifica se il dispositivo è già connesso
                auto it = g_audioPlaybackConnections.find(std::wstring(deviceId));
                if (it != g_audioPlaybackConnections.end())
                {
                    //fprintf(stdout, "   Stato: Connesso\n");
                }
                else
                {
                    //fprintf(stdout, "   Stato: Non connesso\n");
                }

                //fprintf(stdout, "\n");
            }

            // Ottieni il nome del file eseguibile per mostrare l'uso corretto
            WCHAR exePath[MAX_PATH];
            GetModuleFileNameW(NULL, exePath, MAX_PATH);
            WCHAR* exeName = wcsrchr(exePath, L'\\');
            if (exeName) exeName++; else exeName = exePath;

            //fprintf(stdout, "Utilizzo:\n");
            //fprintf(stdout, "  %ls connetti \"Nome Dispositivo\"\n", exeName);
            //fprintf(stdout, "  %ls connect \"Device Name\"\n", exeName);
        }

        //fprintf(stdout, "\nLa lista dei dispositivi e' stata mostrata. Il programma rimane in esecuzione.\n");
        //fprintf(stdout, "Puoi chiudere questa finestra o continuare a usare l'applicazione normalmente.\n");

        // Flush dell'output per assicurarsi che tutto venga scritto
        fflush(stdout);
        fflush(stderr);
        co_return;
    }
    ...
    {
        fprintf(stderr, "Errore durante la ricerca dei dispositivi: %ls\n", ex.message().c_str());
        fflush(stderr);
    }
}
...
void UpdateNotifyIcon()
{
    ...
    LOG_IF_WIN32_ERROR(RegGetValueW(HKEY_CURRENT_USER, LR"(Software\Microsoft\Windows\CurrentVersion\Themes\Personalize)", L"SystemUusesLightTheme", RRF_RT_REG_DWORD, nullptr, &value, &cbValue));
...
// Funzione per parsare la linea di comando
bool ParseCommandLine(LPWSTR lpCmdLine)
{
    ...
    {
        if (_wcsicmp(argv[i], L"connetti") == 0 || _wcsicmp(argv[i], L"connect") == 0)
        {
            // Il prossimo argomento dovrebbe essere il nome del dispositivo
            if (i + 1 < argc)
            {
                g_commandLineDevices.push_back(argv[i + 1]);
                i++; // Salta il nome del dispositivo
                break;
            }
        }
        else if (_wcsicmp(argv[i], L"/connetti") == 0 || _wcsicmp(argv[i], L"/connect") == 0 ||
            _wcsicmp(argv[i], L"-connetti") == 0 || _wcsicmp(argv[i], L"-connect") == 0)
        {
            // Supporto per parametri with slash/dash
            if (i + 1 < argc)
            {
                g_commandLineDevices.push_back(argv[i + 1]);
                i++; // Salta il nome del dispositivo
        ...
        else if (_wcsicmp(argv[i], L"dispositivi") == 0 || _wcsicmp(argv[i], L"devices") == 0 ||
            _wcsicmp(argv[i], L"/dispositivi") == 0 || _wcsicmp(argv[i], L"/devices") == 0 ||
            _wcsicmp(argv[i], L"-dispositivi") == 0 || _wcsicmp(argv[i], L"-devices") == 0 ||
            _wcsicmp(argv[i], L"list") == 0 || _wcsicmp(argv[i], L"lista") == 0)
        {
            // Comando per listare i dispositivi
            g_listDevices = true;
            
        }
        else if (wcschr(argv[i], L':') != nullptr || wcslen(argv[i]) > 3)
        {
            // Se l'argomento sembra un ID dispositivo o un nome, aggiungilo direttamente
            g_commandLineDevices.push_back(argv[i]);
        }
    }

    LocalFree(argv);
    return !g_commandLineDevices.empty() || g_listDevices;
}

// Funzione per elaborare i comandi da riga di comando
void ProcessCommandLine()
{
    if (g_autoConnect && !g_commandLineDevices.empty())
    {
        for (const auto& deviceId : g_commandLineDevices)
        {
    ...
    _In_ int       nCmdShow)
{
    ...
    // Verifica se siamo stati chiamati da una console
    g_hasConsole = HasConsole();
    ...
    // Se è stato richiesto l'elenco dei dispositivi, avvia l'operazione asincrona
    if (g_listDevices)
    {
        ListDevicesAsync(); // Avvia l'operazione asincrona
    }

    // Se abbiamo solo il comando "dispositivi" e nient'altro, possiamo uscire?
    if (g_listDevices && !g_autoConnect && g_commandLineDevices.empty())
    {
        // Continua l'esecuzione normale ma senza interfaccia grafica?
        // In questo caso lasciamo che il programma continui normalmente
    }
    ...
    if (!supported)
    {
        if (g_hasConsole)
        {
            fprintf(stderr, "Unsupported Operating System: AudioPlaybackConnector is not supported on this operating system version.\n");
            fflush(stderr);
        }
        else
        {
    ...
    // Quando la dimensione della finestra padre è 0x0 or invisible, la scala dpi del menu è errata. Qui impostiamo la dimensione della finestra su 1x1 e usiamo WS_EX_LAYERED per far sembrare la finestra invisibile.
    ...
    // Se abbiamo dispositivi da connettere dalla linea di comando, invia un messaggio per elaborarli
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CONNECTDEVICE:
        ...
        // Elabora i comandi da riga di comando se presenti
        if (g_autoConnect && !g_commandLineDevices.empty())
        {
            ProcessCommandLine();
        }
